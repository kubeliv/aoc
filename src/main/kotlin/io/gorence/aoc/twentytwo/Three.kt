...
fun processInput(s: String) {
  val prioritySum = s.split("\n").map(::separate).sum()
  println(prioritySum)
}

fun separate(sequence: String): Int {
  val compartmentSize = sequence.length / 2
  val a = sequence.subSequence(0, compartmentSize)
  val b = sequence.subSequence(compartmentSize, sequence.length)

  // First and second bitmap
  val aB = createLetterBitmap(a.toString())
  val bB = createLetterBitmap(b.toString())

  // Common bitmap
  val cB = (aB and bB)
  println(cB.toString(2).padStart(64, '0'))

  val priority = 0

  for (i in 0..63) {
    val z = cB and (1L shl i)
    if (z > 0) {
      return i + 1
    }
  }
...
fun createLetterBitmap(subsequence: String): Long {
  var bitmap = 0L

  subsequence.toCharArray().forEach {
    bitmap = when (it) {
      in 'a'..'z' -> {
        val k = (it - 'a')
        bitmap or (1L shl k)
...
private const val INPUT = """tdltdtmhlRNCBcwmHr
